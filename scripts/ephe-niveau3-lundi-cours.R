...
# l'estimation est de nouveau un peu étroite
...
rpoints <- st_sample(regions, 700) %>% # des points aléatoires dans la région
  st_sf() %>%  # transformer le shapefile en dataframe
  st_transform(4326)  # le crs pour les coordonnées spatiales
...
dperche1 <- read.table("donnees/richesse_avifaune_perche.txt",header=T,sep="\t")
dperche1$massif <- factor(dperche1$massif)

# Richesse spécifique en fonction des variables voulues, tous massifs confondus
...
# Richesse spécifique en fonction des variables voulues, avec effet fixe massif

mod.perche2<-glm(rs~Hdom+ST+essence+massif,data=dperche1,family=poisson)
summary(mod.perche2) 

# suffisamment de points par massif pour un effet fixe, tous les effets sont bien estimés malgré les hétérogén?ités d'échantillonnage
...
# le warning est dû au fait qu'on a des variables sur de grandes échelles de valeurs. Une solution est de centrer - réduire (recommandé)
...
## modèle à pentes aléatoires : simulation 
...
confint(mod.perche5,method="Wald") # de nouveau : penser à passer en "profile" pour avoir des IC plus corrects, mais !! durée de calcul

# sorties graphiques du modèle ? pentes al?atoires
...
hist(d_pheno$date_debourrement) 

# on ne pourra pas appliquer de transformation dans ce cas (aucune interprétabilité bio + pas de transfo permettant
