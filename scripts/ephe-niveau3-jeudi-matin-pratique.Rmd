---
title: "données opportunistes et indices de diversité"
...
knitr::opts_knit$set(root.dir = 'D:/EPHE_enseignement/Statistiques/certificat/2026/supports/ephe-certificat-stat-3/donnees')
```

## Objectifs

Les indicateurs de diversité des communautés biotiques font partie de la boite à outils classique des experts du suivi de la biodiversité. Nous allons les exploiter pour développer une analyse en autonomie. Le but n'est pas d'aborder la question des indicateurs eux-mêmes, qui nécessiterait une séquence complète, mais de s'entrainer à développer une stratégie d'analyse dans le cadre d'un travail en groupe tel qu'on peut le faire dans un séminaire de recherche ou un groupe de travail. 

Ce cas d'étude permet d'aborder deux questions distinctes :  
* la problématique d'un effort d'échantillonnage hétérogène
* l'analyse des relations entre indicateurs de diversité des communautés et environnement  

Nous nous basons pour cela sur un jeu de données portant sur les reptiles de la région Occitanie, acquis au début des années 2010  (P Geniez & M Cheylan, CEFE-EPHE, base de données Malpolon). Les données sont des observations opportunistes rapportées par des observateurs amateurs ou professionnels et bancarisées après validation. Toute donnée est constituée d'une espèce, d'un jeu de coordonnées géographiques et d'une date, plus des métadonnées éventuelles que nous n'exploiterons pas ici.

L'article d'origine est consultable à : doi:10.1111/ddi.13176 . Nous vous conseillons néanmoins de ne pas le consulter et de ne pas consulter les scripts en amont et en cours d'exercice, le but étant justement de vous entrainer à identifier vous-mêmes  les enjeux et les solutions. A noter que le script R (ephe-niveau3-jeudi-matin.R) et ce Markdown sont légèrement différents sur la forme mais les analyses implémentées sont les mêmes. 
...
```{r}
msp=read.table("reptiles_sites_especes.txt",header=T,sep="\t")
head(msp)
```
...
```{r}
luse=read.table("reptiles_covariables_environnement.txt",header=T,sep="\t")
summary(luse)
```
...
```{r}
grille=read.table("reptiles_grille.txt",header=T,sep="\t")
summary(grille)
```
