...
# séquence: données de distributions spatiales opportunistes
...
plot(resid.sp,
     main = "Lézard ocellé, résidus et localisation des données",
     pch = 21,
     cex = 0.5)
...
t1 <- Sys.time()
mod.rept <- ppm(rept.ppp,
               ~ urb + temp,
               interaction = Geyer(r = 1001, sat = 7),
               eps = 50)
t2 <- Sys.time()
...
plot(mod.rept.urbctt.pred,
     col = h,
     equal.ribbon = T) # on trouve une espèce essentiellement de plaine littorale, comme attendu
...
t1 <- Sys.time()
mod.rept.ech.nogeyer = ppm(rept.ppp,  ~ urb + temp + date + be + elim, , eps =
                             50)
t2 <- Sys.time()
t2 - t1
...
plot(resid.sp.ech,
     main = "Lézard vert, résidus et localisation des données",
     pch = 21,
     cex = 0.5)
...
plot(res.temp.ech.nogeyer, main = "température, avec correction, sans spatial", ylim =
       c(-4, 2))
plot(res.urb.ech.nogeyer, main = "urbanisation, avec correction, sans spatial", ylim =
       c(-4, 2))
...
plot(mod.rept.pred,
     col = h,
     equal.ribbon = T,
     main = "") # les deux prédictions se ressemblent beaucoup, mais il y a des différences locales
...
dat.multsp <- read.csv2(
  "donnees/occurrences_reptiles_occitanie.csv",
  dec = ".",
  header = T,
  fill = T,
  stringsAsFactors = T
...
ppm.multsp <- ppm(
  ...
  ~ marks * urb +
    ...
    be +
    marks * elim,
  interaction = Geyer(r = 980, sat = 48),
  eps = 50
)
t2 <- Sys.time()
...
plot(res_sp$PODMUR)
plot(Smooth(res_sp$PODMUR))
plot(res_sp$CHASTR)
plot(Smooth(res_sp$CHASTR))
plot(res_sp$LACBIL)
plot(Smooth(res_sp$LACBIL))
plot(res_sp$MALMON)
plot(Smooth(res_sp$MALMON))
plot(res_sp$NATMAU)
plot(Smooth(res_sp$NATMAU))
plot(res_sp$PSAALG)
plot(Smooth(res_sp$PSAALG))
plot(res_sp$PSAEDW)
plot(Smooth(res_sp$PSAEDW))
plot(res_sp$RHISCA)
plot(Smooth(res_sp$RHISCA))
plot(res_sp$TARMAU)
plot(Smooth(res_sp$TARMAU))
plot(res_sp$TIMLEP)
plot(Smooth(res_sp$TIMLEP))
plot(res_sp$ZAMLON)
plot(Smooth(res_sp$ZAMLON))
plot(res_sp$NATNAT)
plot(Smooth(res_sp$NATNAT))
plot(res_sp$PODLIO)
plot(Smooth(res_sp$PODLIO))
plot(res_sp$ANGFRA)
plot(Smooth(res_sp$ANGFRA))
