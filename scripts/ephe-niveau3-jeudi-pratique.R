...
# objectif de la séquence : coder une RLQ (nb seul le premier exemple est commenté)
#------------------------------------------------------------------------------#
...
traits <- read.table("donnees/table_traits_lepidos.txt",header=T,sep="\t")
rhopa <- read.table("donnees/table_especes_lepidos.txt",header=T,sep="\t")
habitat <- read.table("donnees/table_habitat_lepidos.txt",header=T,sep="\t",row.names=1)
...
RLQ <- rlq(pca.Rw,afc.L,hs.Qw,scannf=F,nf=2)
...
plot(fourth.rhopalo, alpha = 0.05, stat = "D2") # si un trait est catégoriel, on mesure séparément les associations entre chaque catégorie et les variables environnementales
...
L <- read.table("donnees/Reptiles_matrice_sites_especes.txt",header=T,sep="\t",row.names=1)
R <- read.table("donnees/Reptiles_matrice_habitats.txt",header=T,sep="\t",row.names=1)
Q <- read.table("donnees/Reptiles_matrice_especes_traits.txt",header=T,sep="\t",row.names=1)
...
rlq <- rlq(Rpca,Lcoa, Qhs, nf = 2, scannf=F)
...
NZ_habitats <- read.csv2("donnees/NZ_habitats.csv",header=T, row.names = 1, dec = ".")
NZ_sitsp <- read.csv2("donnees/NZ_points_ecoute.csv",header=T,row.names = 1)
NZ_traits <- read.csv2("donnees/NZ_traits.csv",header=T,stringsAsFactors = T, row.names = 1)
NZ.index <- read.csv2("donnees/NZ_especes_statuts.csv",header=T)
...
rownames(NZ.index) <- NZ.index$acronyme
NZ.index <- NZ.index[rownames(rlq$lQ),]
s.class(rlq$lQ, fac=factor(NZ.index$statut_NZ),col=c("darkred","steelblue","orange"))

## Cas pratique 4 : oiseaux de La Réunion---------------------------------------

# données

acronymes.sp <- read.table("donnees/oiseaux_reunion_statuts.txt",header=T)
traits.reu <- read.table("donnees/oiseaux_lareunion_traits.txt",header=T,row.names=1,sep="\t")
...
habitats.reu <- read.table("donnees/oiseaux_la_reunion_habitats.txt",header=T,row.names=1)
sites.sp.reu <- read.table("donnees/oiseaux_la_reunion.txt",header=T,sep="\t")
